...
function LogoutButton() {
  return (
    <form action={confeitariaLogoutAction}>
      <Button variant="ghost" size="sm" type="submit" className="w-full text-xs text-zinc-400 hover:text-rose-600 gap-1">
    ...
    <div className="flex flex-col md:flex-row h-screen bg-[#FFF9F6] text-zinc-800 overflow-hidden font-sans">
      ...
      <div className="md:hidden flex items-center justify-between px-4 py-3 bg-white border-b border-rose-100 fixed top-0 left-0 right-0 z-40">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <Button variant="ghost" size="icon" className="h-8 w-8 text-rose-950"
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link key={item.href} href={item.href} onClick={() => setMobileMenuOpen(false)}
                  className={`flex items-center gap-3 px-3.5 py-3 rounded-xl text-xs font-semibold transition-all ${isActive ? 'bg-rose-600 text-white shadow-sm' : 'text-zinc-600 hover:bg-rose-50'}`}>
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href}
              className={`flex flex-col items-center gap-1 text-[10px] font-semibold py-1 px-3 rounded-lg transition-all ${isActive ? 'text-rose-600 font-bold' : 'text-zinc-400 hover:text-zinc-600'}`}>
        ...
        <div>
          ...
          <nav className="p-4 space-y-1.5">
            {navItems.map((item) => {
              ...
              return (
                <Link key={item.href} href={item.href}
                  className={`flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-xs font-semibold transition-all ${isActive ? 'bg-rose-600 text-white shadow-sm shadow-rose-200' : 'text-zinc-600 hover:bg-rose-50 hover:text-rose-950'}`}>
                  <item.icon className="h-4 w-4" />
                  {item.label}
                </Link>
              )
